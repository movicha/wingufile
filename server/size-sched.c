...
schedule_pulse (void *vscheduler)
{
    ...
    while (sched->priv->n_running_repo_size_jobs < CONCURRENT_JOBS) {
        ...
        int ret = ccnet_job_manager_schedule_job (sched->winguf->job_mgr,
                                                  compute_repo_size,
...
static gboolean get_head_id (SeafDBRow *row, void *data)
{
    ...
    head_id = winguf_db_row_get_column_text (row, 0);
               ...
               gint64 size)
{
    ...
    trans = winguf_db_begin_transaction (db);
    if (!trans)
        return -1;
    ...
    int n = winguf_db_trans_foreach_selected_row (trans, sql,
                                                get_head_id,
                                                cached_head_id);
    if (n < 0) {
        ret = SET_SIZE_ERROR;
...
get_cached_head_id (SeafDB *db, const char *repo_id)
{
    ...
    snprintf (sql, sizeof(sql), "SELECT head_id FROM RepoSize WHERE repo_id='%s'",
              repo_id);
    return winguf_db_get_string (db, sql);
}
...
retry:
    repo = winguf_repo_manager_get_repo (sched->winguf->repo_mgr, job->repo_id);
    if (!repo) {
        g_warning ("[scheduler] failed to get repo %s.\n", job->repo_id);
        return vjob;
    }

    cached_head_id = get_cached_head_id (sched->winguf->db, job->repo_id);
    if (g_strcmp0 (cached_head_id, repo->head->commit_id) == 0)
        goto out;

    head = winguf_commit_manager_get_commit (sched->winguf->commit_mgr,
                                           repo->head->commit_id);
    if (!head) {
    ...
    size = winguf_fs_manager_get_fs_size (sched->winguf->fs_mgr, head->root_id);
    if (size < 0) {
    ...
    int ret = set_repo_size (sched->winguf->db,
                             job->repo_id,
                             cached_head_id,
                             repo->head->commit_id,
                             size);
