...
winguf_repo_free (SeafRepo *repo)
{
    ...
    if (repo->virtual_info)
        winguf_virtual_repo_info_free (repo->virtual_info);
    g_free (repo);
}
...
winguf_repo_get_commits (SeafRepo *repo)
{
    ...
    if (branches == NULL) {
        winguf_warning ("Failed to get branch list of repo %s.\n", repo->id);
        return NULL;
    }
...
winguf_repo_verify_passwd (SeafRepo *repo, const char *passwd)
{
    ...
    wingufile_generate_enc_key (buf->str, buf->len, repo->enc_version, key, iv);
...
winguf_repo_manager_init (SeafRepoManager *mgr)
{
    ...
    if (create_db_tables_if_not_exist (mgr) < 0) {
        winguf_warning ("[repo mgr] failed to create tables.\n");
        return -1;
    }

    if (winguf_repo_manager_init_merge_scheduler() < 0) {
        winguf_warning ("Failed to init merge scheduler.\n");
        return -1;
    }
...
winguf_repo_manager_get_repo (SeafRepoManager *manager, const gchar *id)
{
    ...
    if (repo_exists_in_db (manager->winguf->db, id, &db_err)) {
...
winguf_repo_manager_get_repo_ex (SeafRepoManager *manager, const gchar *id)
{
    ...
    exists = repo_exists_in_db (manager->winguf->db, id, &db_err);

    if (db_err) {
        ret = winguf_repo_new(id, NULL, NULL);
...
{
    gboolean db_err = FALSE;
    return repo_exists_in_db (manager->winguf->db, id, &db_err);
}
...
save_branch_repo_map (SeafRepoManager *manager, SeafBranch *branch)
{
    ...
    if (winguf_db_type(winguf->db) == SEAF_DB_TYPE_PGSQL) {
        gboolean err;
        snprintf (sql, sizeof(sql),
                  "SELECT repo_id FROM RepoHead WHERE repo_id='%s'",
                  branch->repo_id);
        if (winguf_db_check_for_existence (winguf->db, sql, &err))
            snprintf(sql, sizeof(sql),
        ...
        if (err)
            return -1;
        return winguf_db_query(winguf->db, sql);
    } else {
                  ...
                  SeafBranch *branch)
{
    ...
    commit = winguf_commit_manager_get_commit (manager->winguf->commit_mgr,
                                             branch->commit_id);
    if (!commit) {
        winguf_warning ("Commit %s is missing\n", branch->commit_id);
...
load_repo (SeafRepoManager *manager, const char *repo_id, gboolean ret_corrupt)
{
    ...
    if (!repo) {
        winguf_warning ("[repo mgr] failed to alloc repo.\n");
        return NULL;
    }
    ...
    repo->virtual_info = winguf_repo_manager_get_virtual_repo_info (manager, repo_id);
                ...
                GError **error)
{
    ...
    if (winguf_db_query (mgr->winguf->db, sql) < 0) {
        winguf_warning ("failed to add repo token. repo = %s, email = %s\n",
                      repo_id, email);
                                       ...
                                       gint64 sync_time)
{
    ...
    char *esc_peer_name = winguf_db_escape_string (mgr->winguf->db, peer_name);
    g_string_printf (sql,
                                          ...
                                          const char *token)
{
    ...
    snprintf (sql, sizeof(sql),
              ...
              token);
    return winguf_db_check_for_existence (mgr->winguf->db, sql, &db_error);
}
                                ...
                                GError **error)
{
    ...
    token_owner = winguf_repo_manager_get_email_by_token (mgr, repo_id, token);
    if (!token_owner || strcmp (user, token_owner) != 0) {
        winguf_warning ("Requesting user is %s, token owner is %s, "
                      "refuse to delete token %.10s.\n", user, token_owner, token);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL, "Permission denied");
        return -1;
    }
...
collect_repo_token (SeafDBRow *row, void *data)
{
    ...
    repo_owner = winguf_db_row_get_column_text (row, 1);
    email = winguf_db_row_get_column_text (row, 2);
    token = winguf_db_row_get_column_text (row, 3);

    peer_id = winguf_db_row_get_column_text (row, 4);
    peer_ip = winguf_db_row_get_column_text (row, 5);
...
fill_in_token_info (GList *info_list)
{
    ...
    for (ptr = info_list; ptr; ptr = ptr->next) {
        ...
        repo = winguf_repo_manager_get_repo (winguf->repo_mgr,
                                           wingufile_repo_token_info_get_repo_id(info));
        if (repo)
                                    ...
                                    GError **error)
{
    ...
    if (!repo_exists_in_db (mgr->winguf->db, repo_id, &db_err)) {
        if (db_err) {
            g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL, "DB error");
        }
        return NULL;
    ...
    int n_row = winguf_db_foreach_selected_row (mgr->winguf->db, sql->str,
                                              collect_repo_token, &ret_list);
    if (n_row < 0) {
        winguf_warning ("DB error when get token info for repo %.10s.\n",
                      repo_id);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL, "DB error");
    }
                                             ...
                                             GError **error)
{
    ...
    int n_row = winguf_db_foreach_selected_row (mgr->winguf->db, sql->str,
                                              collect_repo_token, &ret_list);
    if (n_row < 0) {
        winguf_warning ("DB error when get token info for email %s.\n",
                      email);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL, "DB error");
    }
...
get_email_by_token_cb (SeafDBRow *row, void *data)
{
    ...
    *email_ptr = g_strdup(winguf_db_row_get_column_text (row, 0));
                                      ...
                                      const char *token)
{
    ...
    winguf_db_foreach_selected_row (winguf->db, buf->str,
                                          ...
                                          const char *repo_id)
{
    ...
    snprintf (sql, sizeof(sql),
              ...
              r_repo_id);
    per_repo_days = winguf_db_get_int (mgr->winguf->db, sql);
...
get_owner (SeafDBRow *row, void *data)
{
    ...
    *owner_id = g_strdup(winguf_db_row_get_column_text (row, 0));
    /* There should be only one result. */
    return FALSE;
}
                                  ...
                                  const char *repo_id)
{
    ...
    if (winguf_db_foreach_selected_row (mgr->winguf->db, sql,
                                      get_owner, &ret) < 0) {
        winguf_warning ("Failed to get owner id for repo %s.\n", repo_id);
        return NULL;
    }
                                      ...
                                      const char *email)
{
    ...
    for (ptr = id_list; ptr; ptr = ptr->next) {
        ...
        SeafRepo *repo = winguf_repo_manager_get_repo (mgr, repo_id);
        if (repo != NULL)
                                       ...
                                       const char *ap)
{
    ...
    if (winguf_repo_manager_query_access_property (mgr, repo_id) == NULL) {
    ...
    if (winguf_db_query (mgr->winguf->db, sql) < 0) {
        winguf_warning ("DB error when set access property for repo %s, %s.\n", repo_id, ap);
        return -1;
    }
...
get_ap (SeafDBRow *row, void *data)
{
    ...
    *ap = g_strdup (winguf_db_row_get_column_text (row, 0));
...
winguf_repo_manager_query_access_property (SeafRepoManager *mgr, const char *repo_id)
{
    ...
    if (winguf_db_foreach_selected_row (mgr->winguf->db, sql, get_ap, &ret) < 0) {
        winguf_warning ("DB error when get access property for repo %s.\n", repo_id);
        return NULL;
    }
...
get_group_ids_cb (SeafDBRow *row, void *data)
{
    ...
    int group_id = winguf_db_row_get_column_int (row, 0);
                                      ...
                                      GError **error)
{
    ...
    if (winguf_db_foreach_selected_row (mgr->winguf->db, sql, get_group_ids_cb,
                                       &group_ids) < 0) {
...
get_group_perms_cb (SeafDBRow *row, void *data)
{
    ...
    perm->group_id = winguf_db_row_get_column_int (row, 0);
    const char *permission = winguf_db_row_get_column_text(row, 1);
                                          ...
                                          GError **error)
{
    ...
    if (winguf_db_foreach_selected_row (mgr->winguf->db, sql, get_group_perms_cb,
                                      &group_perms) < 0) {
...
get_group_repoids_cb (SeafDBRow *row, void *data)
{
    ...
    char *repo_id = g_strdup ((const char *)winguf_db_row_get_column_text (row, 0));
                                     ...
                                     GError **error)
{
    ...
    if (winguf_db_foreach_selected_row (mgr->winguf->db, sql, get_group_repoids_cb,
                                      &repo_ids) < 0)
        return NULL;
...
get_group_repos_cb (SeafDBRow *row, void *data)
{
    ...
    const char *user_name = winguf_db_row_get_column_text (row, 2);
    const char *permission = winguf_db_row_get_column_text (row, 3);
                                            ...
                                            GError **error)
{
    ...
    if (winguf_db_foreach_selected_row (mgr->winguf->db, sql, get_group_repos_cb,
                                      &repos) < 0)
        return NULL;
...
get_group_repo_owner (SeafDBRow *row, void *data)
{
    ...
    *share_from = g_strdup (winguf_db_row_get_column_text (row, 0));
    /* There should be only one result. */
    return FALSE;
}
                                        ...
                                        GError **error)
{
    ...
    if (winguf_db_foreach_selected_row (mgr->winguf->db, sql,
        ...
        winguf_warning ("DB error when get repo share from for repo %s.\n",
                   repo_id);
        return NULL;
    }
...
collect_public_repos (SeafDBRow *row, void *data)
{
    ...
    repo_id = winguf_db_row_get_column_text (row, 0);
    owner = winguf_db_row_get_column_text (row, 1);
                                ...
                                const char *repo_id)
{
    ...
    snprintf (sql, sizeof(sql),
              ...
              repo_id);
    return winguf_db_get_int (mgr->winguf->db, sql);
}
...
int
winguf_repo_manager_set_org_repo (SeafRepoManager *mgr,
                                int org_id,
                                const char *repo_id,
                                const char *user)
{
                                            ...
                                            GError **error)
{
    ...
    if (winguf_db_foreach_selected_row (mgr->winguf->db, sql,
                                      get_group_repo_owner, &ret) < 0) {
        winguf_warning ("DB error when get repo owner from for org repo %s.\n",
                   repo_id);
        return NULL;
    }
...
GList *
winguf_repo_manager_list_org_inner_pub_repos (SeafRepoManager *mgr,
                                            int org_id)
{
    GList *ret = NULL;
    char sql[256];
...
winguf_repo_generate_magic (SeafRepo *repo, const char *passwd)
{
    ...
    wingufile_generate_enc_key (buf->str, buf->len, CURRENT_ENC_VERSION, key, iv);
                                   ...
                                   GError **error)
{
    ...
    if (winguf_repo_manager_set_repo_owner (mgr, repo_id, owner_email) < 0) {
        winguf_warning ("Failed to set repo owner.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                                   ...
                                   GError **error)
{
    ...
    if (winguf_repo_manager_set_org_repo (mgr, org_id, repo_id, user) < 0) {
        winguf_warning ("Failed to set org repo.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
