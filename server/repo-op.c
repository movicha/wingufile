...
dup_winguf_dirent (const SeafDirent *dent)
{
    return winguf_dirent_new (dent->id, dent->mode, dent->name);
}

static inline GList *
dup_wingufdir_entries (const GList *entries)
{
    const GList *p;
    ...
    for (p = entries; p; p = p->next) {
        dent = p->data;
        newentries = g_list_prepend (newentries, dup_winguf_dirent(dent));
    }
                     ...
                     SeafDirent *newdent)
{
    ...
    olddir = winguf_fs_manager_get_wingufdir_sorted(winguf->fs_mgr, dir_id);
    if (!olddir)
        return NULL;
    ...
    if (*to_path == '\0') {
        ...
        newentries = dup_wingufdir_entries (olddir->entries);

        newentries = g_list_insert_sorted (newentries,
                                           dup_winguf_dirent(newdent),
                                           compare_dirents);

        newdir = winguf_dir_new (NULL, newentries, 0);
        winguf_dir_save (winguf->fs_mgr, newdir);
        id = g_strndup (newdir->dir_id, 41);
        id[40] = '\0';
        winguf_dir_free (newdir);
    ...
    if (id != NULL) {
        ...
        new_entries = dup_wingufdir_entries (olddir->entries);
        newdir = winguf_dir_new (NULL, new_entries, 0);
        winguf_dir_save (winguf->fs_mgr, newdir);
                   ...
                   int  *mode)
{
    ...
    dir = winguf_fs_manager_get_wingufdir_by_path (winguf->fs_mgr, root_id,
                                               parent_dir, NULL);
    if (!dir) {
        winguf_warning ("parent_dir %s doesn't exist.\n", parent_dir);
        return FALSE;
    }
...
/**
  ...
  Put a file:
  1. find parent wingufdir
  2. add a new dirent to parent wingufdir
  2. recursively update all wingufdir in the path, in a bottom-up manner
  3. commit it
  ...
  Rename a file/dir:
  1. find parent wingufdir
  2. update this wingufdir with the old dirent replaced by a new dirent.
  3. recursively update all wingufdir in the path
...
#define GET_REPO_OR_FAIL(repo_var,repo_id)                              \
    do {                                                                \
        repo_var = winguf_repo_manager_get_repo (winguf->repo_mgr, (repo_id)); \
        if (!(repo_var)) {                                              \
            winguf_warning ("Repo %s doesn't exist.\n", (repo_id));       \
            g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS, "Invalid repo"); \
            ret = -1;                                                   \
            goto out;                                                   \
        }                                                               \
    } while (0);
...
#define GET_COMMIT_OR_FAIL(commit_var,commit_id)                        \
    do {                                                                \
        commit_var = winguf_commit_manager_get_commit(winguf->commit_mgr, (commit_id)); \
        if (!(commit_var)) {                                            \
            winguf_warning ("commit %s doesn't exist.\n", (commit_id));   \
            g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS, "Invalid commit"); \
            ret = -1;                                                   \
            goto out;                                                   \
        }                                                               \
    } while (0);
...
#define MAX_RETRY_COUNT 3
    ...
    if (!repo) {
        winguf_warning ("Repo %s doesn't exist.\n", repo_id);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL, "Invalid repo");
        ret = -1;
        goto out;
    }

    /* Create a new commit pointing to new_root. */
    new_commit = winguf_commit_new(NULL, repo->id, new_root,
                                 user, EMPTY_SHA1,
                                 desc, 0);
    new_commit->parent_id = g_strdup (base->commit_id);
    winguf_repo_to_commit (repo, new_commit);

    if (winguf_commit_manager_add_commit (winguf->commit_mgr, new_commit) < 0) {
...
retry:
    current_head = winguf_commit_manager_get_commit (winguf->commit_mgr,
                                                   repo->head->commit_id);
    if (!current_head) {
        winguf_warning ("Failed to find head commit of %s.\n", repo_id);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL, "Invalid repo");
        ret = -1;
        goto out;
    }
    ...
    if (strcmp (base->commit_id, current_head->commit_id) != 0) {
        ...
        if (winguf_merge_trees (3, roots, &opt) < 0) {
            winguf_warning ("Failed to merge.\n");
            g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                         "Internal error");
            ret = -1;
            goto out;
        ...
        winguf_debug ("Number of dirs visted in merge %.8s: %d.\n",
                    repo_id, opt.visit_dirs);

        merged_commit = winguf_commit_new(NULL, repo->id, opt.merged_tree_root,
                                        user, EMPTY_SHA1,
        ...
        winguf_repo_to_commit (repo, merged_commit);

        if (winguf_commit_manager_add_commit (winguf->commit_mgr, merged_commit) < 0) {
            winguf_warning ("Failed to add commit.\n");
            g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                         "Failed to add commit");
            ret = -1;
        ...
        winguf_commit_ref (new_commit);
        merged_commit = new_commit;
    }

    winguf_branch_set_commit(repo->head, merged_commit->commit_id);
                                                   ...
                                                   current_head->commit_id) < 0)
    {
        ...
        repo = NULL;
        winguf_commit_unref (current_head);
        current_head = NULL;
        winguf_commit_unref (merged_commit);
        merged_commit = NULL;
...
out:
    winguf_commit_unref (new_commit);
    winguf_commit_unref (current_head);
    winguf_commit_unref (merged_commit);
    winguf_repo_unref (repo);
    return ret;
                             ...
                             GError **error)
{
    ...
    if (g_access (temp_file_path, R_OK) != 0) {
        winguf_warning ("[post file] File %s doesn't exist or not readable.\n",
                      temp_file_path);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS,
                     "Invalid input file");
        return -1;
    ...
    if (should_ignore_file (file_name, NULL)) {
        winguf_warning ("[post file] Invalid filename %s.\n", file_name);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS,
                     "Invalid filename");
        ret = -1;
        goto out;
    ...
    if (strstr (parent_dir, "//") != NULL) {
        winguf_warning ("[post file] parent_dir cantains // sequence.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS,
                     "Invalid parent dir");
        ret = -1;
        goto out;
    ...
    if (repo->encrypted) {
        ...
        if (winguf_passwd_manager_get_decrypt_key_raw (winguf->passwd_mgr,
                                                     repo_id, user,
                                                     key, iv) < 0) {
            winguf_warning ("Passwd for repo %s is not set.\n", repo_id);
            g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                         "Passwd is not set");
            ret = -1;
            goto out;
        }
        crypt = wingufile_crypt_new (repo->enc_version, key, iv);
    }

    if (winguf_fs_manager_index_blocks (winguf->fs_mgr, temp_file_path,
                                      sha1, crypt) < 0) {
        winguf_warning ("failed to index blocks");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
    ...
    new_dent = winguf_dirent_new (hex, S_IFREG, file_name);

    root_id = do_post_file (head_commit->root_id, canon_path, new_dent);
    if (!root_id) {
        winguf_warning ("[post file] Failed to put file.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                     "Failed to put file");
        ret = -1;
        goto out;
    ...
    winguf_repo_manager_merge_virtual_repo (mgr, repo_id, NULL);

out:
    if (repo)
        winguf_repo_unref (repo);
    if (head_commit)
        winguf_commit_unref(head_commit);
    if (new_dent)
        g_free (new_dent);
    g_free (root_id);
    g_free (canon_path);
...
add_new_entries (GList **entries, GList *filenames, GList *id_list)
{
         ...
         ptr1 = ptr1->next, ptr2 = ptr2->next)
    {
        ...
        if (i <= 16) {
            newdent = winguf_dirent_new (id, S_IFREG, unique_name);
...
json_to_file_list (const char *files_json)
{
    ...
    if (error) {
        winguf_warning ("Failed to load file list from json.\n");
        g_error_free (error);
        return NULL;
    }
                                    ...
                                    GError **error)
{
    ...
    if (!filenames || !paths) {
        winguf_warning ("[post files] Invalid filenames or paths.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS, "Invalid files");
        ret = -1;
        goto out;
    }
    ...
    for (ptr = filenames; ptr; ptr = ptr->next) {
        ...
        if (should_ignore_file (filename, NULL)) {
            winguf_warning ("[post files] Invalid filename %s.\n", filename);
            g_set_error (error, WINGUFILE_DOMAIN, POST_FILE_ERR_FILENAME,
                         "%s", filename);
            ret = -1;
            goto out;
        }
    ...
    if (*to_path == '\0') {
        ...
        for (p = olddir->entries; p != NULL; p = p->next) {
            ...
            if (strcmp(old->name, filename) != 0) {
                new = winguf_dirent_new (old->id, old->mode, old->name);
                            ...
                            GError **error)
{
    ...
    if (!check_file_exists(head_commit->root_id, canon_path, file_name, &mode)) {
        winguf_warning ("[del file] target file %s/%s does not exist, skip\n",
    ...
    if (!root_id) {
        winguf_warning ("[del file] Failed to del file.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                     "Failed to del file");
        ret = -1;
        goto out;
    ...
    winguf_repo_manager_cleanup_virtual_repos (mgr, repo_id);
                    ...
                    GError **error)
{
    ...
    head_commit = winguf_commit_manager_get_commit(winguf->commit_mgr,
                                                 repo->head->commit_id);
    if (!head_commit) {
        winguf_warning ("commit %s doesn't exist.\n", repo->head->commit_id);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS, "Invalid commit");
    ...
    dir = winguf_fs_manager_get_wingufdir_by_path (winguf->fs_mgr, head_commit->root_id,
                                              path, NULL);
    if (!dir) {
        winguf_warning ("dir %s doesn't exist in repo %s.\n", path, repo->id);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS, "Invalid dir");
        goto out;
    }
    ...
    for (p = dir->entries; p; p = p->next) {
        ...
        if (r == 0) {
            dent = winguf_dirent_new (d->id, d->mode, d->name);
            break;
        }
    }
                       ...
                       GError **error)
{
    ...
    if (!root_id) {
        winguf_warning ("[cp file] Failed to cp file.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                     "Failed to cp file");
        ret = -1;
        goto out;
                             ...
                             GError **error)
{
    ...
    dst_dent = winguf_dirent_new (src_dent->id, src_dent->mode, dst_filename);
    ...
    winguf_repo_manager_merge_virtual_repo (mgr, dst_repo_id, NULL);

out:
    if (src_repo)
        winguf_repo_unref (src_repo);
    if (dst_repo)
        winguf_repo_unref (dst_repo);
    if (dst_head_commit)
        winguf_commit_unref(dst_head_commit);
    if (src_canon_path)
                     ...
                     GError **error)
{
    ...
    if (!root_id_after_put) {
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS, "move file failed");
        ret = -1;
        goto out;
    }
                             ...
                             GError **error)
{
    ...
    if (src_repo == dst_repo) {
        ...
        winguf_repo_manager_cleanup_virtual_repos (mgr, src_repo_id);
        winguf_repo_manager_merge_virtual_repo (mgr, src_repo_id, NULL);
    } else {
        ...
        if (winguf_repo_manager_del_file (mgr, src_repo_id, src_path,
...
out:
    if (src_repo) winguf_repo_unref (src_repo);
    if (dst_repo) winguf_repo_unref (dst_repo);

    if (dst_head_commit) winguf_commit_unref(dst_head_commit);
                            ...
                            GError **error)
{
    ...
    if (should_ignore_file (new_dir_name, NULL)) {
        winguf_warning ("[post dir] Invalid dir name %s.\n", new_dir_name);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS,
                     "Invalid dir name");
        ret = -1;
        goto out;
    ...
    if (!new_dent) {
        new_dent = winguf_dirent_new (EMPTY_SHA1, S_IFDIR, new_dir_name);
    }

    root_id = do_post_file (head_commit->root_id, canon_path, new_dent);
    if (!root_id) {
        winguf_warning ("[put dir] Failed to put dir.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                     "Failed to put dir");
        ret = -1;
        goto out;
                                   ...
                                   GError **error)
{
    ...
    if (should_ignore_file (new_file_name, NULL)) {
        winguf_warning ("[post file] Invalid file name %s.\n", new_file_name);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS,
                     "Invalid file name");
        ret = -1;
        goto out;
    ...
    if (!new_dent) {
        new_dent = winguf_dirent_new (EMPTY_SHA1, S_IFREG, new_file_name);
    }

    root_id = do_post_file (head_commit->root_id, canon_path, new_dent);
    if (!root_id) {
        winguf_warning ("[put dir] Failed to create empty file dir.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                     "Failed to put dir");
        ret = -1;
        goto out;
    ...
    if (*to_path == '\0') {
        ...
        for (p = olddir->entries; p != NULL; p = p->next) {
            ...
            if (strcmp(old->name, oldname) != 0) {
                newentries = g_list_prepend (newentries, dup_winguf_dirent(old));
            } else {
                newdent = winguf_dirent_new (old->id, old->mode, newname);
            }
        }
                               ...
                               GError **error)
{
    ...
    if (should_ignore_file (newname, NULL)) {
        winguf_warning ("[rename file] Invalid filename %s.\n", newname);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS,
                     "Invalid filename");
        ret = -1;
        goto out;
    ...
    if (*to_path == '\0') {
        ...
        for (p = olddir->entries; p; p = p->next) {
            ...
            if (strcmp(dent->name, newdent->name) == 0) {
                newentries = g_list_prepend (newentries, dup_winguf_dirent(newdent));
            } else {
                newentries = g_list_prepend (newentries, dup_winguf_dirent(dent));
            }
        }
                            ...
                            GError **error)
{
    ...
    if (g_access (temp_file_path, R_OK) != 0) {
        winguf_warning ("[put file] File %s doesn't exist or not readable.\n",
                      temp_file_path);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS,
                     "Invalid input file");
        return -1;
    ...
    if (should_ignore_file (file_name, NULL)) {
        winguf_warning ("[put file] Invalid filename %s.\n", file_name);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS,
                     "Invalid filename");
        ret = -1;
        goto out;
    ...
    if (strstr (parent_dir, "//") != NULL) {
        winguf_warning ("[put file] parent_dir cantains // sequence.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_ARGS,
                     "Invalid parent dir");
        ret = -1;
        goto out;
    ...
    old_file_id = winguf_fs_manager_path_to_obj_id (winguf->fs_mgr,
                                                   head_commit->root_id,
                                                   fullpath, NULL, NULL);
    ...
    if (!root_id) {
        winguf_warning ("[put file] Failed to put file.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                     "Failed to put file");
        ret = -1;
        goto out;
                              ...
                              GError **error)
{
    ...
    new_dent = winguf_dirent_new (new_dir_id, S_IFDIR, dirname);

    root_id = do_put_file (head_commit->root_id, canon_path, new_dent);
    if (!root_id) {
        winguf_warning ("[update dir] Failed to put file.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                     ...
                     GError **error)
{
    ...
    *skipped = FALSE;
    ...
    newdent = winguf_dirent_new (file_id, S_IFREG, new_file_name);
                   ...
                   GError **error)
{
    ...
    dir = winguf_fs_manager_get_wingufdir_by_path (winguf->fs_mgr, root_id, path, error);
    if (*error) {
        if (g_error_matches(*error, WINGUFILE_DOMAIN, SEAF_ERR_PATH_NO_EXIST)) {
            /* path does not exist */
            g_clear_error(error);
            return FALSE;
        } else {
                               ...
                               GError **error)
{
    ...
    if (!old_commit) {
        ...
        if (strcmp(old_commit->repo_id, repo_id) != 0) {
            g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_BAD_COMMIT,
                         "bad commit id");
            ret = -1;
            goto out;
        }
    ...
    if (!canon_path) {
        ...
        revert_to_file_id = winguf_fs_manager_get_wingufile_id_by_path (
                    winguf->fs_mgr, old_commit->root_id, canon_path, error);
        if (*error) {
            winguf_warning ("[revert file] error: %s\n", (*error)->message);
            g_clear_error (error);
            g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                         "internal error");
            ret = -1;
    ...
    if (!root_id) {
        winguf_warning ("[revert file] Failed to revert file.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
...
out:
    ...
    if (head_commit)
        winguf_commit_unref (head_commit);
    if (old_commit)
        winguf_commit_unref (old_commit);
            ...
            GError **error)
{
    ...
    *skipped = FALSE;
    ...
    newdent = winguf_dirent_new (dir_id, S_IFDIR, new_dir_name);
    new_root_id = do_post_file (root_id, parent_dir, newdent);
                              ...
                              GError **error)
{
    ...
    if (!canon_path) {
        ...
        revert_to_dir_id = winguf_fs_manager_get_wingufdir_id_by_path (
                    winguf->fs_mgr, old_commit->root_id, canon_path, error);
        if (*error) {
            winguf_warning ("[revert dir] error: %s\n", (*error)->message);
            g_clear_error (error);
            g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                         "internal error");
            ret = -1;
    ...
    if (!root_id) {
        winguf_warning ("[revert dir] Failed to revert dir.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
...
collect_file_revisions (SeafCommit *commit, void *vdata, gboolean *stop)
{
    ...
    parent_commit = winguf_commit_manager_get_commit (winguf->commit_mgr,
                                             commit->parent_id);
    if (!parent_commit) {
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
    ...
    if (commit->second_parent_id) {
        parent_commit2 = winguf_commit_manager_get_commit (winguf->commit_mgr,
...
out:
    ...
    if (parent_commit) winguf_commit_unref (parent_commit);
    if (parent_commit2) winguf_commit_unref (parent_commit2);
                                       ...
                                       GError **error)
{
    ...
    data.truncate_time = winguf_repo_manager_get_repo_truncate_time (mgr, repo_id);
                                            ...
                                            GError **error)
{
    ...
    head_commit = winguf_commit_manager_get_commit (winguf->commit_mgr, repo->head->commit_id);
    if (!head_commit) {
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
    ...
    */
    ...
    if (!winguf_commit_manager_traverse_commit_tree_with_limit (winguf->commit_mgr,
                                ...
                                (CommitTraverseFunc)collect_files_last_modified,
                                                              limit, &data)) {
        winguf_warning ("error when travsersing commits: %s\n", (*error)->message);
        g_clear_error (error);
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL,
                     "failed to traverse commit of repo %s", repo_id);
        goto out;
...
retry:
    ...
    new_commit = winguf_commit_new (NULL, repo->id, commit->root_id,
    ...
    winguf_branch_set_commit (repo->head, new_commit->commit_id);
    if (winguf_branch_manager_test_and_update_branch (winguf->branch_mgr,
                                                    repo->head,
                                                    new_commit->parent_id) < 0)
    {
        winguf_warning ("[revert] Concurrent branch update, retry.\n");
        winguf_repo_unref (repo);
                   ...
                   SeafCommit *parent)
{
    ...
    entry = g_object_new (WINGUFILE_TYPE_DELETED_ENTRY,
                        ...
                        GHashTable *entries)
{
    ...
    d1 = winguf_fs_manager_get_wingufdir_sorted (winguf->fs_mgr, root1);
    if (!d1) {
        winguf_warning ("Failed to find dir %s.\n", root1);
        return -1;
    }
    d2 = winguf_fs_manager_get_wingufdir_sorted (winguf->fs_mgr, root2);
    if (!d2) {
        winguf_warning ("Failed to find dir %s.\n", root2);
        winguf_dir_free (d1);
        return -1;
...
collect_deleted (SeafCommit *commit, void *vdata, gboolean *stop)
{
    ...
    p1 = winguf_commit_manager_get_commit (commit->manager, commit->parent_id);
    if (!p1) {
    ...
    if (commit->second_parent_id) {
        p2 = winguf_commit_manager_get_commit (commit->manager,
                                             commit->second_parent_id);
        if (!p2) {
            winguf_warning ("Failed to find commit %s.\n",
                          commit->second_parent_id);
...
remove_existing (gpointer key, gpointer value, gpointer user_data)
{
    ...
    char *obj_id = winguf_fs_manager_path_to_obj_id (winguf->fs_mgr, head->root_id,
                                                    path, &mode_out, NULL);
    if (obj_id == NULL)
        return FALSE;
    g_free (obj_id);
...
filter_out_existing_entries (GHashTable *entries, const char *head_id)
{
    ...
    if (!head) {
        winguf_warning ("Failed to find head commit %s.\n", head_id);
        return -1;
    }
                                       ...
                                       GError **error)
{
    ...
    truncate_time = winguf_repo_manager_get_repo_truncate_time (mgr, repo_id);
    if (truncate_time == 0)
        return NULL;
...
get_commit(SeafRepo *repo, const char *branch_or_commit)
{
    ...
    b = winguf_branch_manager_get_branch (winguf->branch_mgr, repo->id,
                                        branch_or_commit);
    if (!b) {
        if (strcmp(branch_or_commit, "HEAD") == 0)
            c = winguf_commit_manager_get_commit (winguf->commit_mgr,
                                                repo->head->commit_id);
        else
            c = winguf_commit_manager_get_commit (winguf->commit_mgr,
                                                branch_or_commit);
    } else {
        c = winguf_commit_manager_get_commit (winguf->commit_mgr, b->commit_id);
    }
...
GList *
winguf_repo_diff (SeafRepo *repo, const char *old, const char *new, char **error)
{
    ...
    if (old == NULL || old[0] == '\0') {
        ...
        c1 = winguf_commit_manager_get_commit (winguf->commit_mgr,
                                             c2->parent_id);
    } else {
