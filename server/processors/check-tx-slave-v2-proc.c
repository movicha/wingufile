...
release_resource(CcnetProcessor *processor)
{
    ...
    CCNET_PROCESSOR_CLASS (wingufile_check_tx_slave_v2_proc_parent_class)->release_resource (processor);
}
...
decrypt_token (CcnetProcessor *processor)
{
    ...
    if (hex_len % 2 != 0) {
        winguf_warning ("[check tx slave v2] invalid length of encrypted token\n"); 
        ret = -1;
        goto out;
    }

    token = winguf_repo_manager_get_decrypted_token (winguf->repo_mgr,
                                                   priv->token,
                                                   priv->session_key);
    if (token)
        goto found;
    ...
    wingufile_generate_enc_key (priv->session_key,
    ...
    if (wingufile_decrypt (&token, &token_len, encrypted_token,
        ...
        winguf_warning ("[check tx slave v2] failed to decrypt token\n");
        ret = -1;
        goto out;
    }

    /* Add to cache. */
    winguf_repo_manager_add_decrypted_token (winguf->repo_mgr,
                                           priv->token,
                                           priv->session_key,
                                           token);
...
start (CcnetProcessor *processor, int argc, char **argv)
{
    ...
    if (!peer || !peer->session_key) {
        winguf_warning ("[check tx slave v2] session key of peer %.10s is null\n",
                      processor->peer_id);
