...
#define GET_PRIV(o)  \
   (G_TYPE_INSTANCE_GET_PRIVATE ((o), WINGUFILE_TYPE_RECVCOMMIT_V3_PROC, RecvcommitPriv))
...
G_DEFINE_TYPE (WingufileRecvcommitV3Proc, wingufile_recvcommit_v3_proc, CCNET_TYPE_PROCESSOR)
...
release_resource (CcnetProcessor *processor)
{
    ...
    CCNET_PROCESSOR_CLASS (wingufile_recvcommit_v3_proc_parent_class)->release_resource (processor);
}

static void
wingufile_recvcommit_v3_proc_class_init (WingufileRecvcommitV3ProcClass *klass)
{
...
static void
wingufile_recvcommit_v3_proc_init (WingufileRecvcommitV3Proc *processor)
{
}
...
receive_commit (CcnetProcessor *processor, char *content, int clen)
{
    ...
    winguf_debug ("[recvcommit] recv commit object %.8s\n", pack->id);
                           ...
                           char *content, int clen)
{
    ...
    case RECV_OBJECT:
        if (strncmp(code, SC_OBJECT, 3) == 0) {
            ...
            winguf_debug ("[recvcommit] Recv commit end.\n");
