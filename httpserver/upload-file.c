...
#define DEBUG_FLAG WINGUFILE_DEBUG_HTTP
#include "log.h"
                          ...
                          int error_code)
{
    ...
    if (filename) {
        ...
        snprintf(url, 1024, "%s/repo/upload_error/%s?p=%s&fn=%s&err=%d",
                 winguhub_url, repo_id, escaped_path, escaped_fn, error_code);
    } else {
...
upload_cb(evhtp_request_t *req, void *arg)
{
    ...
    wingufile_post_multi_files (rpc_client,
                              fsm->repo_id,
                              parent_dir,
                              filenames_json,
                              tmp_files_json,
...
upload_api_cb(evhtp_request_t *req, void *arg)
{
    ...
    char *new_file_ids = wingufile_post_multi_files (rpc_client,
                                              fsm->repo_id,
                                              parent_dir,
                                              filenames_json,
                                              tmp_files_json,
...
update_api_cb(evhtp_request_t *req, void *arg)
{
    ...
    new_file_id = wingufile_put_file (rpc_client,
                                    fsm->repo_id,
                                    (char *)(fsm->files->data),
                                    parent_dir,
                                    filename,
                    ...
                    char **user)
{
    ...
    *repo_id = g_strdup (wingufile_web_access_get_repo_id (webaccess));
    *user = g_strdup (wingufile_web_access_get_username (webaccess));
