...
next:
    ...
    if (!data->handle) {
        ...
        if (data->crypt) {
            if (wingufile_decrypt_init (&data->ctx,
                                      data->crypt->version,
        ...
        SeafileCryptKey *crypt_key)
{
    ...
    if (crypt_key != NULL) {
        ...
        crypt = wingufile_crypt_new (repo->enc_version, enc_key, enc_iv);
        g_free (key_hex);
        g_free (iv_hex);
    }
...
access_cb(evhtp_request_t *req, void *arg)
{
    ...
    repo_id = wingufile_web_access_get_repo_id (webaccess);
    id = wingufile_web_access_get_obj_id (webaccess);
