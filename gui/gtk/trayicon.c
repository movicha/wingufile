...
void reset_trayicon_and_tip(SeafileTrayIcon *icon)
{
    ...
    wingufile_trayicon_set_tooltip (icon, tip);
}
...
tray_icon_create_menu (SeafileTrayIcon *icon)
{
    ...
    filename = ccnet_file_lookup ("wingufile-trayicon.ui", "gtk");
    builder = gtk_builder_get_all_widgets (filename,
    /* builder = gtk_builder_get_all_widgets ("wingufile-trayicon.ui", */
...
wingufile_tray_icon_init (SeafileTrayIcon *icon)
{
    ...
    desktop = g_getenv ("XDG_CURRENT_DESKTOP");
    if (g_strcmp0(desktop, "Unity") == 0) {
        AppIndicator *app_icon = app_indicator_new("wingufile",
                                               ICON_STATUS_UP,
...
wingufile_trayicon_new (GtkWindow *window)
{
    ...
    wingufile_trayicon_set_icon (icon, ICON_STATUS_UP);
