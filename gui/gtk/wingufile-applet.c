...
int start_web_server ()
{
    ...
    if (spawn_process("wingufile-web start") < 0) {
        applet_warning ("Failed to start wingufile web\n");
        applet_exit(-1);
    }
...
int
set_wingufile_auto_start(int on)
{
    return 0;
}
...
wingufile_applet_init (SeafileApplet *applet)
{
    ...
    applet->icon = wingufile_trayicon_new ();
}
...
main (int argc, char **argv)
{
    /* init i18n */
    setlocale (LC_ALL, "");
    bindtextdomain(GETTEXT_PACKAGE, WINGUFILE_LOCALE_DIR);
    ...
    gtk_icon_theme_append_search_path (gtk_icon_theme_get_default(),
                                       PKGDATADIR);
    gtk_window_set_default_icon_name ("wingufile");
...
stop_web_server ()
{
    if (applet->web_status != WEB_NOT_STARTED) {
        if (system("wingufile-web stop") < 0) {
...
trayicon_set_ccnet_state (int state)
{
    ...
    wingufile_trayicon_set_icon (applet->icon, name);
}

void
trayicon_notify (char *title, char *buf)
{
    wingufile_trayicon_notify (applet->icon, title, buf);
}
...
do_rotate ()
{
    ...
    static char *names[] = { WINGUFILE_TRANFER_1, WINGUFILE_TRANFER_2,
                             WINGUFILE_TRANFER_3, WINGUFILE_TRANFER_4 };

    int index = nth_trayicon % G_N_ELEMENTS(names);
    char *name = names[index];
    
    wingufile_trayicon_set_icon (icon, name);
...
void trayicon_set_tip (char *tip)
{
    ...
    wingufile_trayicon_set_tooltip (applet->icon, tip);
}
