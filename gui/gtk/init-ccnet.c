...
int show_init_wingufile_window ()
{
    gchar           *filename;
    gchar           *dir;
    int              response;
    GtkButton       *wingufiledir_filechooser;
    GtkLabel        *wingufiledir_label;

    filename = ccnet_file_lookup ("init-wingufile-window.ui", "gtk");
    builder = gtk_builder_get_all_widgets (
        filename,
        "dialog", &dialog,
        "wingufiledir_label", &wingufiledir_label,
        "wingufiledir_filechooser", &wingufiledir_filechooser,
        NULL);
    g_free (filename);
...
again:
    ...
    dir = gtk_file_chooser_get_filename (
        (GtkFileChooser *)wingufiledir_filechooser);
    if (!dir) {
    ...
    applet->wingufile_dir = g_build_filename(dir, "wingufile-data", NULL);
    gtk_widget_destroy (GTK_WIDGET(dialog));
    return 0;
}
