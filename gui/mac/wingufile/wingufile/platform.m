...
int start_wingufile_daemon (void)
{
...
{
#ifdef DEBUG
    system ("/usr/local/bin/wingufile-web start");
#else
    NSLog (@" start web server ...\n");
    NSString *path = [[NSBundle mainBundle] pathForResource: @"wingufileweb.app" ofType: nil];
...
{
#ifdef DEBUG
    system ("/usr/local/bin/wingufile-web stop");
...
int show_init_wingufile_window (void)
{
    AppDelegate *delegate = [[NSApplication sharedApplication] delegate];
    return [delegate show_initwingufile_window];
}
...
int is_wingufile_daemon_running (void)
{
    AppDelegate *delegate = [[NSApplication sharedApplication] delegate];
...
int set_wingufile_auto_start (const int on)
{
    AppDelegate *delegate = [[NSApplication sharedApplication] delegate];
    if (on)
