...
- (void)applicationDidFinishLaunching: (NSNotification *)aNotification
{
    ...
    wingufile_applet_start (0, NULL);

    wingufile_set_wingufilefolder_icns ();
    NSString *path = [[NSString alloc] initWithUTF8String: applet->wingufile_worktree];
    [self add_dir_to_sidebar: path];
    set_visibility_for_file (applet->wingufile_dir, YES, NO);
...
{
    NSLog (@"wingufile will terminate\n");
    stop_ccnet ();
    stop_web_server();
}
...
- (IBAction)initwingufile_ok: (id)sender
{
    ...
    applet->wingufile_worktree = g_build_filename (cpath, "wingufile", NULL);
    applet->wingufile_dir = g_build_filename (cpath, "wingufile", "wingufile-data", NULL);
}
...
- (void)awakeFromNib
{
    ...
    inited = 1;
    if (is_process_already_running ("wingufile")) {
        NSLog (@"wingufile.app has already been running, just quit %d\n", getpid ());
        exit (1);
    }
    ...
    repoImage  = [[NSImage alloc] initWithContentsOfFile: [bundle pathForResource: @"wingufilerepo" ofType: @"icns"]];
