...
#define GETTEXT_PACKAGE "wingufile"
...
get_wingufile_auto_start()
{
    ...
    RegCloseKey(hKey);
    if (result != ERROR_SUCCESS) {
        /* wingufile applet auto start no set  */
        return 0;
    }
...
set_applet_wd()
{
    ...
    wingufile_bin_dir = g_path_get_dirname (module_path_utf8);
    if (!wingufile_bin_dir) {
        return -1;
    }

    bindir_w = wchar_from_utf8 (wingufile_bin_dir);
    if (!bindir_w) {
        return -1;
    }
...
static void
wingufile_applet_init (HINSTANCE hInstance)
{
...
WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    char *wingufile_locale_dir = g_build_filename (wingufile_bin_dir,
                                                 "i18n", NULL);
    ...
    snprintf(cmdbuf, sizeof(cmdbuf), "wingufile-applet.exe %s", lpCmdLine);
