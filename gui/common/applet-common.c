...
#ifdef __APPLE__
#include "../mac/wingufile/wingufile/platform.h"
#endif

#define WINGUFILE_OFFICIAL_ADDR "cloud.wingufile.com.cn:10001"
#define WINGUFILE_INI "wingufile.ini"

#if !defined(WINGUFILE_CLIENT_VERSION)
...
save_wingufile_dir()
{
    ...
    char *wingufile_dir = applet->wingufile_dir;

    path = g_build_filename(config_dir, WINGUFILE_INI, NULL);
...
static char *
get_wingufile_dir(const char *config_dir)
{
    FILE *fp = NULL;
    char buf[4096];

    char *fname = g_build_filename (config_dir, WINGUFILE_INI, NULL);
...
init_ccnet ()
{
    ...
    applet_message ("starting wingufile-applet "WINGUFILE_CLIENT_VERSION"\n");
...
init_wingufile ()
{
    applet->wingufile_dir = get_wingufile_dir(applet->config_dir);
    
    if (!applet->wingufile_dir
        || !g_file_test(applet->wingufile_dir, G_FILE_TEST_EXISTS)) {
        
        first_use = TRUE;
        if (show_init_wingufile_window() < 0)
            applet_exit (1);
        else 
            save_wingufile_dir();
    }
    ...
    if (parent_name && (strcmp(parent_name, "Seafile") == 0)) {
        ...
        /* For wingufile version < 0.9.4 in windows and all versions in linux
              applet->wingufile_dir:       /data/wingufile-data
              applet->wingufile_worktree:  /data/wingufile
         */
        applet->wingufile_worktree = g_build_filename (parent_dir, "wingufile", NULL);
    }
    g_free (parent_dir);
    g_free (parent_name);
#else
    if (!applet->wingufile_worktree) {
        applet->wingufile_worktree = g_path_get_dirname (applet->wingufile_dir);
    }
#endif
    g_setenv("WINGUFILE_WORKTREE", applet->wingufile_worktree, 1);
}
...
static void
handle_wingufile_notification (char *type, char *content)
{
    char buf[1024];
    ...
    if (strcmp(type, "transfer") == 0) {
        ...
        *p = '\0';
        ...
        wingufile_set_repofolder_icns (content);
    } else if  (strcmp(type, "repo.unsetwktree") == 0) {
        wingufile_unset_repofolder_icns (content);
    }
#endif
}

static int
parse_wingufile_notification (char *msg, char **type, char **body)
{
    if (!msg)
        return -1;
...
static void mq_cb (CcnetMessage *msg, void *data)
{
    ...
    if (IS_APP_MSG(msg, "wingufile.heartbeat")) {
        applet->last_heartbeat = time(NULL);
    } else if (IS_APP_MSG(msg, "wingufile.notification")) {
        if (parse_wingufile_notification (msg->body, &type, &content) < 0)
            return;

        handle_wingufile_notification (type, content);
    }
}
...
connect_to_daemon (void)
{
    ...
    applet_init_wingufile_rpc (client);
...
int
wingufile_applet_start (int argc, char **argv)
{
    int c;
    char *temp_confdir;
    const char *env;
...
int is_repo_path_allowed(const char *path) {
    ...
    len = strlen(applet->wingufile_dir);
    g_assert(applet->wingufile_dir[len-1] != '/');
    snprintf(buf2, 4095, "%s/", applet->wingufile_dir);
...
void wingufile_disable_auto_sync (void)
{
    SetAutoSyncData *sdata = g_new0 (SetAutoSyncData, 1);
    sdata->disable = TRUE;
    call_wingufile_disable_auto_sync (set_auto_sync_cb, sdata);
}

void wingufile_enable_auto_sync (void)
{
    SetAutoSyncData *sdata = g_new0 (SetAutoSyncData, 1);
    sdata->disable = FALSE;
    call_wingufile_enable_auto_sync (set_auto_sync_cb, sdata);
}
