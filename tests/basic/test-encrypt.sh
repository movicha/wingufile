...
{    
    # create a repo
    ${wingufile_app} -c ${conf1} create  --encrypt --passwd=1234 test-repo test > /dev/null
    sleep 3
...
repo_id=`ls ${worktree}/wt1/`
echo "+++ upload ${repo_id}"
${wingufile_app} -c ${conf1} upload ${repo_id} local master
sleep 10

echo "+++ fetch ${repo_id}"
${wingufile_app} -c ${conf3} fetch ${repo_id} master master
sleep 10

echo "+++ checkout"
${wingufile_app} -c ${conf3} branch add ${repo_id} local master
${wingufile_app} -c ${conf3} set-passwd ${repo_id} 1234
${wingufile_app} -c ${conf3} checkout ${repo_id} local
