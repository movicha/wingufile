...
int main (int argc, char **argv)
{
    ...
    if (argc != 3) {
        printf ("wingufile-fmt-test wingufile original-file\n");
        exit (1);
    }

    wingufile_path = argv[1];
    file_path = argv[2];

    fd = g_open (wingufile_path, O_RDONLY | O_BINARY);
    if (fd < 0) {
        printf ("Failed to open wingufile.\n");
        exit (1);
    }
