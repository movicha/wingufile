...
static int
write_wingufile (SeafFSManager *fs_mgr,
                ...
                SeafileCrypt *crypt)
{
    ...
    if (crypt != NULL) {
        ...
        */
        ...
        /* decrypt the block */
        int ret = wingufile_decrypt (&dec_out,
                                   &dec_out_len,
                                   blk_content,
                                   bmd->size, 
                                   crypt);
                     ...
                     gboolean write_data)
{
    ...
    if (crypt != NULL && chunk->len) {
        ...
        ret = wingufile_encrypt (&encrypted_buf, /* output */
                               &enc_len,      /* output len */
                              ...
                              SeafileCrypt *crypt)
{
    ...
    if (sb.st_size == 0) {
        ...
        cdc.write_block = wingufile_write_chunk;
    ...
    if (write_wingufile (mgr, &cdc) < 0) {
        g_warning ("Failed to write wingufile for %s.\n", file_path);
        return -1;
    }
...
wingufile_from_data (const char *id, const void *data, int len)
{
    ...
    wingufile->n_blocks = n_blocks;

    wingufile->blk_sha1s = g_new0 (char*, wingufile->n_blocks);
    const unsigned char *blk_sha1_ptr = ondisk->block_ids;
    int i;
    for (i = 0; i < wingufile->n_blocks; ++i) {
        char *blk_sha1 = g_new0 (char, 41);
        wingufile->blk_sha1s[i] = blk_sha1;
...
wingufile_free (Seafile *wingufile)
{
    ...
    if (wingufile->blk_sha1s) {
        for (i = 0; i < wingufile->n_blocks; ++i)
            g_free (wingufile->blk_sha1s[i]);
        g_free (wingufile->blk_sha1s);
    }
...
seaf_fs_manager_get_wingufile (SeafFSManager *mgr, const char *file_id)
{
    ...
    wingufile = wingufile_from_data (file_id, data, len);
    g_free (data);
     ...
     */
    wingufile_ref (wingufile);
    g_hash_table_insert (mgr->priv->wingufile_cache, g_strdup(file_id), wingufile);
#endif
                ...
                void *user_data, gboolean *stop)
{
    ...
    if (type == SEAF_METADATA_TYPE_FILE) {
        ...
        for (i = 0; i < wingufile->n_blocks; ++i)
            block_list_insert (bl, wingufile->blk_sha1s[i]);
