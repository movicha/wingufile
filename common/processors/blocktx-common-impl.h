...
#ifdef SEND
    g_string_printf (buf, "remote %s wingufile-recvblock %s", 
                     processor->peer_id, task->session_token);
#else
    if (task->session_token)
        g_string_printf (buf, "remote %s wingufile-putblock %s", 
                         processor->peer_id, task->session_token);
    else
        g_string_printf (buf, "remote %s wingufile-putblock", 
                         processor->peer_id);
#endif
...
#ifdef SEND
    CCNET_PROCESSOR_CLASS(wingufile_sendblock_proc_parent_class)->release_resource (processor);
#else
    CCNET_PROCESSOR_CLASS(wingufile_getblock_proc_parent_class)->release_resource (processor);
#endif
}
...
#ifndef SEND
    CCNET_PROCESSOR_CLASS(wingufile_recvblock_proc_parent_class)->release_resource (processor);
#else
    CCNET_PROCESSOR_CLASS(wingufile_putblock_proc_parent_class)->release_resource (processor);
#endif
}
