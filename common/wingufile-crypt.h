...
/*
  ...
  The function pair "wingufile_encrypt/wingufile_decrypt" are used to
  encrypt/decrypt data in the wingufile system, using AES 128 bit ecb
  algorithm provided by openssl.
*/  

#ifndef _WINGUFILE_CRYPT_H
#define _WINGUFILE_CRYPT_H
...
/*  
  ...
  *data_out, with out_len set to its length. On failure, -1 is returned
  ...
  wingufile_encrypt is always a multiple of BLK_SIZE(16 Bytes), and the
  input length of wingufile_decrypt *must* be a multiple of BLK_SIZE.
  
*/

int
wingufile_generate_enc_key (const char *data_in, int in_len, int version,
                          unsigned char *key, unsigned char *iv);

int
wingufile_encrypt (char **data_out,
                 int *out_len,
                 const char *data_in,
                 const int in_len,
                 SeafileCrypt *crypt);
...
int
wingufile_decrypt (char **data_out,
                 int *out_len,
                 const char *data_in,
                 const int in_len,
                 SeafileCrypt *crypt);
