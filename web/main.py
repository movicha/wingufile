...
urls = (
    ...
    '/i18n/', 'i18n',
    '/wingufile_access_check/', 'wingufile_access_check',
    '/open-local-file/', 'open_local_file',
    '/wingufile_rpc_version/', 'wingufile_rpc_version',
    )
...
def prepare_repo_info(repo):
    ...
    repo.sync_info = wingufile_rpc.get_repo_sync_info(repo.props.id)
...
class repos:
    ...
    def GET(self):
        ...
        global lang_in_use
        if not lang_in_use:
            lang_in_use = wingufile_rpc.get_config('lang_in_use')
            if not lang_in_use:
                wingufile_rpc.set_config('lang_in_use', DEFAULT_LANG)
...
class repo:
    ...
    def show_repo(self, repo_id):
        repo = wingufile_rpc.get_repo(repo_id)
        if not repo:
        ...
        relay_addr = wingufile_rpc.get_repo_relay_address(repo_id)
        relay_port = wingufile_rpc.get_repo_relay_port(repo_id)
...
class repo_transfer:
    ...
    def GET(self):
        inputs = web.webapi.input(repo='')
        task = {}
        t = wingufile_rpc.find_transfer_task(inputs.repo)
        if t:
...
class repo_sync_status:
    ...
    def GET(self):
        ...
        t = wingufile_rpc.get_repo_sync_task(inputs.repo)
        if t:
            ...
            # No sync task yet: wingufile maybe have just been started 
            sync_status['state'] = 'waiting for sync'

        auto_sync_enabled = wingufile_rpc.is_auto_sync_enabled()
...
class repo_operation:
    ...
    def perform_operation_get(self, op, repo_id):
        ...
        if op == 'sync':
            try:
                wingufile_rpc.sync(repo.props.id, None)
            except:
                pass
            ...
            new_commit = wingufile_rpc.get_commit(inputs.new)
            if inputs.old != '':
                old_commit = wingufile_rpc.get_commit(inputs.old)
            else:
                old_commit = None
            ...
            dirs = wingufile_rpc.list_dir_by_path(inputs.commit_id, inputs.path.encode('utf-8'))
            navs = get_dir_nav_links(repo, inputs.commit_id, inputs.path)
            try:
                commit = wingufile_rpc.get_commit(inputs.commit_id)
            except SearpcError:
            ...
            try:
                wingufile_rpc.remove_repo(repo_id)
            except:
                pass
            raise web.seeother('/repos/')
            ...
            auto_sync = {}
            try:
                wingufile_rpc.set_repo_property(repo_id, "auto-sync", "true")
            except:
                pass
            ...
            auto_sync = {}
            try:
                wingufile_rpc.set_repo_property(repo_id, "auto-sync", "false")
            except:
                pass
    ...
    def perform_operation_post(self, op, repo_id):
        ...
        if op == 'modify-relay':
            ...
            if relay_id != repo.props.relay_id:
                wingufile_rpc.set_repo_property(repo.props.id,
                                              "relay-id", relay_id)
        elif op == 'set-passwd':
            passwd = web.webapi.input(passwd="").passwd
            if passwd:
                wingufile_rpc.set_repo_passwd(repo.props.id, passwd)

        elif op == 'edit-relay':
            inputs = web.webapi.input(relay_addr='', relay_port='')
            if inputs.relay_addr and inputs.relay_port:
                wingufile_rpc.update_repo_relay_info(repo_id,
                                                   inputs.relay_addr,
                                                   inputs.relay_port)
...
class CloneTasks:
    ...
    def GET(self):
        ...
        if inputs.op and inputs.repo_id:
            if inputs.op == "remove":
                wingufile_rpc.remove_clone_task(inputs.repo_id)
            elif inputs.op == "cancel":
                wingufile_rpc.cancel_clone_task(inputs.repo_id)
...
class clone_tasks:
    ...
    def GET(self):
        ts = []
        tasks = wingufile_rpc.get_clone_tasks()
        for task in tasks:
            t = {}
            ...
            if task.props.state == "fetch":
                tx_task = wingufile_rpc.find_transfer_task(task.props.repo_id)
                t['tx_block_done'] = tx_task.props.block_done
                t['tx_block_total'] = tx_task.props.block_total
            elif task.props.state == "checkout":
                checkout_task = wingufile_rpc.get_checkout_task(task.props.repo_id)
...
class repo_download:
    ...
    def GET(self):
        ...
        if wingufile_rpc.get_repo(inputs.repo_id):
        ...
        wt_parent = get_default_wingufile_worktree ()
    ...
    def POST(self):
        ...
        try:
            wingufile_rpc.download (inputs.repo_id, inputs.relay_id,
            ...
            if e.msg == 'Invalid local directory':
                ...
                error_msg = _('The local directory you chose cannot be under or includes a system directory of wingufile.')
...
class repo_sync:
    ...
    def POST(self):
        ...
        try:
            wingufile_rpc.clone (repo_id, inputs.relay_id,
                               inputs.repo_name.encode('utf-8'),
...
class settings_page:
    ...
    def POST(self):
        ...
        if inputs.notify_sync != current_prefs['notify_sync']:
            wingufile_rpc.set_config('notify_sync', inputs.notify_sync)
        ...
        if upload_limit != current_prefs['upload_limit']:
            wingufile_rpc.set_upload_rate_limit(upload_limit)

        if download_limit != current_prefs['download_limit']:
            wingufile_rpc.set_download_rate_limit(download_limit)
...
class i18n:
    def GET(self):
        ...
        wingufile_rpc.set_config('lang_in_use', lang_in_use)
...
# for winguhub repo download
class wingufile_access_check:
    """For winguhub to check whether local wingufile is started when downloading a
...
class wingufile_rpc_version:
    """For the server to query current wingufile client rpc version"""
    def GET(self):
        version = 1
...
class open_local_file:
    """
    handle jsonp ajax cross domain 'open-local-file' request from winguhub
    """
    def GET(self):
