...
class PubReposFetch:
    ...
    def POST(self):
        ...
        op = inputs.op
        if op == 'fetch':
            wingufile_rpc.fetch(repo_id, peer.props.id, 'fetch_head', 'master', token)
        else:
            wingufile_rpc.synclan(repo_id, peer.props.id, token)
...
class repo_checkout_status:
    def GET(self):
        ...
        task = None
        if repo_id:
            task = wingufile_rpc.get_checkout_task(repo_id)
        ttask = {}
        if task:
...
class pubrepo_op_status:
    def GET(self):
        ...
        if inputs.op == 'fetch':
            t = wingufile_rpc.find_transfer_task(inputs.repo_id)
            fetch_task = {}
        ...
        if inputs.op == 'sync':
            slist = wingufile_rpc.get_sync_task_list()
            sync_task = {}
...
class pubrepos:
    ...
    def POST(self):
        ...
        if not token:
            token = 'default'
        if (inputs.op == 'fetch'):
            wingufile_rpc.fetch(inputs.repo, inputs.peer, 'fetch_head', 'master', token)
        else:
            wingufile_rpc.synclan(inputs.repo, inputs.peer, token)
...
class pubrepo_operation:
    ...
    def perform_operation_get(self, op, peer_id, repo_id):
        ...
        if op == 'diff':
            ...
            wingufile_rpc.synclan(repo_id, peer_id, inputs.token)
            raise web.seeother('/repos/tasks/')
