...
wingufile_rpc = wingufile.RpcClient(pool, req_pool=True)
wingufile_threaded_rpc = wingufile.ThreadedRpcClient(pool)
...
def get_repos():
    ...
    """
    return wingufile_rpc.get_repo_list(-1, -1)

def get_repo(repo_id):
    return wingufile_rpc.get_repo(repo_id)

def get_commits(repo_id, offset, limit):
    """Get commit lists."""
    return wingufile_rpc.get_commit_list(repo_id, offset, limit)


def checkout(repo_id, commit_id):
    return wingufile_rpc.checkout(repo_id, commit_id)

def get_branches(repo_id):
    """Get branches of a given repo"""
    return wingufile_rpc.branch_gets(repo_id)
...
def get_diff(repo_id, arg1, arg2):
    ...
    diff_result = wingufile_rpc.get_diff(repo_id, arg1, arg2)
    if not diff_result:
        return lists
...
def list_dir(root_id):
    dirent_list = wingufile_rpc.list_dir(root_id);
...
def get_wingufile_config(key, default_value):
    value = wingufile_rpc.get_config(key)
    if value is None:
        wingufile_rpc.set_config(key, default_value)
        return default_value
    else:
        return value

def get_wingufile_config_int(key, default_value):
    try:
        value = wingufile_rpc.get_config_int(key)
...
def get_current_prefs():
    ...
    """
    prefs = {}
    prefs['notify_sync'] = get_wingufile_config('notify_sync', 'on')
    prefs['auto_start'] = 'on' if applet_rpc.get_auto_start() == 1 else 'off'
    prefs['encrypt_channel'] = get_ccnet_config('encrypt_channel', 'off')
    prefs['upload_limit'] = get_wingufile_config_int('upload_limit', 0)
    prefs['download_limit'] = get_wingufile_config_int('download_limit', 0)
    return prefs
