...
'''This script would guide the wingufile admin to setup wingufile with MySQL'''
...
class Utils(object):
    ...
    def welcome():
        ...
        welcome_msg = '''\
-----------------------------------------------------------------
This script will guide you to setup your wingufile server using MySQL.
...
-----------------------------------------------------------------''' % SERVER_MANUAL_HTTP
    ...
    def get_python_executable():
        ...
        variable which is set in setup-wingufile-mysql.sh
...
class EnvManager(object):
    ...
    def __init__(self):
        ...
        self.bin_dir = os.path.join(self.install_path, 'wingufile', 'bin')
    ...
    def check_pre_condiction(self):
        ...
        paths = [
            os.path.join(self.install_path, 'wingufile'),
            os.path.join(self.install_path, 'winguhub'),
    ...
    def get_winguhub_env(self):
        '''Prepare for winguhub syncdb'''
        env = dict(os.environ)
        env['CCNET_CONF_DIR'] = ccnet_config.ccnet_dir
        env['WINGUFILE_CONF_DIR'] = wingufile_config.wingufile_dir
        self.setup_python_path(env)
        return env

    def setup_python_path(self, env):
        '''And PYTHONPATH and CCNET_CONF_DIR/WINGUFILE_CONF_DIR to env, which is
        needed by winguhub
        ...
        extra_python_path = [
            ...
            os.path.join(install_path, 'winguhub', 'thirdpart'),

            os.path.join(install_path, 'wingufile/lib/python2.6/site-packages'),
            os.path.join(install_path, 'wingufile/lib64/python2.6/site-packages'),
            os.path.join(install_path, 'wingufile/lib/python2.7/site-packages'),
            os.path.join(install_path, 'wingufile/lib64/python2.7/site-packages'),
        ]
    ...
    def get_binary_env(self):
        '''Set LD_LIBRARY_PATH for wingufile server executables'''
        env = dict(os.environ)
        lib_dir = os.path.join(self.install_path, 'wingufile', 'lib')
        lib64_dir = os.path.join(self.install_path, 'wingufile', 'lib64')
...
class AbstractConfigurator(object):
    '''Abstract Base class for ccnet/wingufile/winguhub/db configurator'''
    def __init__(self):
        pass
...
class AbstractDBConfigurator(AbstractConfigurator):
    ...
    def __init__(self):
        ...
        self.wingufile_mysql_user = ''
        self.wingufile_mysql_password = ''

        self.ccnet_db_name = ''
        self.wingufile_db_name = ''
        self.winguhub_db_name = ''

        self.winguhub_admin_email = ''
        self.winguhub_admin_password = ''
    ...
    def ask_use_existing_db():
        ...
        question = '''\
-------------------------------------------------------
Please choose a way to initialize wingufile databases:
-------------------------------------------------------
'''

        note = '''\
[1] Create new ccnet/wingufile/winguhub databases
[2] Use existing ccnet/wingufile/winguhub databases
'''
    ...
    def create_winguhub_admin(self):
        try:
            conn = MySQLdb.connect(host=self.mysql_host,
                                   port=self.mysql_port,
                                   user=self.wingufile_mysql_user,
                                   passwd=self.wingufile_mysql_password,
...
CREATE TABLE IF NOT EXISTS EmailUser (id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT, email VARCHAR(255), passwd CHAR(64), is_staff BOOL NOT NULL, is_active BOOL NOT NULL, ctime BIGINT, UNIQUE INDEX (email)) ENGINE=INNODB'''
        ...
        sql = '''REPLACE INTO EmailUser(email, passwd, is_staff, is_active, ctime) VALUES ('%s', '%s', 1, 1, 0)''' \
              % (winguhub_config.admin_email, winguhub_config.hashed_admin_password())
...
class NewDBConfigurator(AbstractDBConfigurator):
    '''Handles the case of creating new mysql databases for ccnet/wingufile/winguhub'''
    def __init__(self):
    ...
    def ask_questions(self):
        ...
        self.ask_wingufile_mysql_user_password()

        self.ask_db_names()

    def generate(self):
        if not self.mysql_user_exists(self.wingufile_mysql_user):
            self.create_user()
    ...
    def ask_wingufile_mysql_user_password(self):
        def validate(user):
            if user == 'root':
                self.wingufile_mysql_password = self.root_password
            else:
                ...
                self.wingufile_mysql_password = password

            return user


        question = 'Enter the name for mysql user of wingufile. It would be created if not exists.'
        key = 'mysql user for wingufile'
        default = 'root'
        self.wingufile_mysql_user = Utils.ask_question(question,
                                                     key=key,
                                                     default=default,
                                                     validate=validate)
    ...
    def ask_db_names(self):
        ...
        self.wingufile_db_name = self.ask_db_name('wingufile-server', 'wingufile-db')
        self.winguhub_db_name = self.ask_db_name('winguhub', 'winguhub-db')
    ...
    def create_user(self):
        ...
        sql = '''CREATE USER '%s'@'localhost' IDENTIFIED BY '%s' ''' \
              % (self.wingufile_mysql_user, self.wingufile_mysql_password)

        try:
            cursor.execute(sql)
        except Exception, e:
            if isinstance(e, MySQLdb.OperationalError):
                Utils.error('Failed to create mysql user %s: %s' % (self.wingufile_mysql_user, e.args[1]))
            else:
                Utils.error('Failed to create mysql user %s: %s' % (self.wingufile_mysql_user, e))
        finally:
            cursor.close()
    ...
    def grant_db_permission(self, db_name):
        ...
        sql = '''GRANT ALL PRIVILEGES ON `%s`.* to `%s` ''' \
              % (db_name, self.wingufile_mysql_user)
    ...
    def create_databases(self):
        ...
        self.create_db(self.wingufile_db_name)
        self.create_db(self.winguhub_db_name)

        if self.wingufile_mysql_user != 'root':
            self.grant_db_permission(self.ccnet_db_name)
            self.grant_db_permission(self.wingufile_db_name)
            self.grant_db_permission(self.winguhub_db_name)


class ExistingDBConfigurator(AbstractDBConfigurator):
    '''Handles the case of use existing mysql databases for ccnet/wingufile/winguhub'''
    def __init__(self):
        AbstractDBConfigurator.__init__(self)
    ...
    def ask_questions(self):
        ...
        self.wingufile_db_name = self.ask_db_name('wingufile')
        self.winguhub_db_name = self.ask_db_name('winguhub')
    ...
    def ask_existing_mysql_user_password(self):
        ...
        question = 'Which mysql user to use for wingufile?'
        key = 'mysql user for wingufile'
        self.wingufile_mysql_user = Utils.ask_question(question,
                                                     key=key,
                                                     validate=validate)
    ...
    def check_user_db_access(self, db_name):
        user = self.wingufile_mysql_user
        password = self.wingufile_mysql_password
...
class CcnetConfigurator(AbstractConfigurator):
    ...
    def generate_db_conf(self):
        ...
        config.set(db_section, 'USER', db_config.wingufile_mysql_user)
        config.set(db_section, 'PASSWD', db_config.wingufile_mysql_password)
...
class SeafileConfigurator(AbstractConfigurator):
    ...
    def generate(self):
        ...
        time.sleep(1)
        self.generate_db_conf()
        self.write_wingufile_ini()
        print 'done'

    def generate_db_conf(self):
        wingufile_conf = os.path.join(self.wingufile_dir, 'wingufile.conf')
        config = Utils.read_config(wingufile_conf)
        # [database]
        # type=
        # host=
        # user=
        ...
        config.set(db_section, 'port', db_config.mysql_port)
        config.set(db_section, 'user', db_config.wingufile_mysql_user)
        config.set(db_section, 'password', db_config.wingufile_mysql_password)
        config.set(db_section, 'db_name', db_config.wingufile_db_name)

        Utils.write_config(config, wingufile_conf)

    def ask_wingufile_dir(self):
        def validate(path):
            if os.path.exists(path):
        ...
        default = os.path.join(env_mgr.top_dir, 'wingufile-data')
        self.wingufile_dir = Utils.ask_question(question,
                                              key=key,
                                              note=note,
                                              default=default,
                                              validate=validate)
    ...
    def ask_port(self):
        ...
        question = 'Which port do you want to use for the wingufile server?'
        key = 'wingufile server port'
        default = 12001
        self.port = Utils.ask_question(question,
                                       key=key,
                                       default=default,
    ...
    def ask_httpserver_port(self):
        def validate(port):
            ...
            if port == wingufile_config.port:
                raise InvalidAnswer('%s is used by wingufile server, choose another one' \
                                    % Utils.highlight(port))

            return port

        question = 'Which port do you want to use for the wingufile httpserver?'
        key = 'wingufile httpserver port'
        default = 8082
    ...
    def write_wingufile_ini(self):
        wingufile_ini = os.path.join(ccnet_config.ccnet_dir, 'wingufile.ini')
        with open(wingufile_ini, 'w') as fp:
            fp.write(self.wingufile_dir)
...
class SeahubConfigurator(AbstractConfigurator):
    ...
    def generate(self):
        '''Generating winguhub_settings.py'''
        print 'Generating winguhub configuration ...\n'
        time.sleep(1)
        winguhub_settings_py = os.path.join(env_mgr.top_dir, 'winguhub_settings.py')
        with open(winguhub_settings_py, 'w') as fp:
...
'''
        text = template % dict(name=db_config.winguhub_db_name,
                               username=db_config.wingufile_mysql_user,
                               password=db_config.wingufile_mysql_password,
    ...
    def do_syncdb(self):
        print '----------------------------------------'
        print 'Now creating winguhub database tables ...\n'
        print '----------------------------------------'
        env = env_mgr.get_winguhub_env()
        cwd = os.path.join(env_mgr.install_path, 'winguhub')
        argv = [
        ...
        if Utils.run_argv(argv, cwd=cwd, env=env) != 0:
            Utils.error("Failed to create winguhub databases")
    ...
    def prepare_avatar_dir(self):
        ...
        try:
            media_dir = os.path.join(env_mgr.install_path, 'winguhub', 'media')
            orig_avatar_dir = os.path.join(media_dir, 'avatars')

            winguhub_data_dir = os.path.join(env_mgr.top_dir, 'winguhub-data')
            dest_avatar_dir = os.path.join(winguhub_data_dir, 'avatars')

            if os.path.exists(dest_avatar_dir):
                return

            if not os.path.exists(winguhub_data_dir):
                os.mkdir(winguhub_data_dir)

            shutil.move(orig_avatar_dir, dest_avatar_dir)
            os.symlink('../../../winguhub-data/avatars', orig_avatar_dir)
        except Exception, e:
            Utils.error('Failed to prepare winguhub avatars dir: %s' % e)
...
def report_config():
    ...
    wingufile data dir:       %(wingufile_dir)s
    wingufile port:           %(wingufile_port)s
    ...
    wingufile database:       %(wingufile_db_name)s
    winguhub database:        %(winguhub_db_name)s
...
'''
    config = {
        ...
        'wingufile_dir' :         wingufile_config.wingufile_dir,
        'wingufile_port' :        wingufile_config.port,
        'httpserver_port' :     wingufile_config.httpserver_port,

        'admin_email' :         winguhub_config.admin_email,


        'use_existing_db':       'use exising' if db_config.use_existing_db else 'create new',
        'ccnet_db_name':        db_config.ccnet_db_name,
        'wingufile_db_name':      db_config.wingufile_db_name,
        'winguhub_db_name':       db_config.winguhub_db_name,
        'db_user':              db_config.wingufile_mysql_user
    }
...
def main():
    ...
    wingufile_config.ask_questions()
    winguhub_config.ask_questions()
    ...
    wingufile_config.generate()
    winguhub_config.generate()

    winguhub_config.do_syncdb()
    winguhub_config.prepare_avatar_dir()
    db_config.create_winguhub_admin()
...
-----------------------------------------------------------------
Your wingufile server configuration has been finished successfully.
-----------------------------------------------------------------

run wingufile server:     ./wingufile.sh { start | stop | restart }
run winguhub  server:     ./winguhub.sh  { start <port> | stop | restart <port> }
...
port of wingufile server:       %(wingufile_port)s
port of wingufile httpserver:   %(httpserver_port)s
port of winguhub:               8000
