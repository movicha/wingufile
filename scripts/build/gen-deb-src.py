...
def remove_unused_files():
    srcdir = os.path.join(conf[CONF_BUILDDIR], 'wingufile-%s' % conf[CONF_VERSION])
...
def gen_tarball():
    output = os.path.join(conf[CONF_OUTPUTDIR], 'wingufile-latest.tar.gz')
    dirname = 'wingufile-%s' % conf[CONF_VERSION]
...
def uncompress_wingufile():
    src = os.path.join(conf[CONF_BUILDDIR], 'wingufile-%s' % conf[CONF_WINGUFILE_VERSION])
    dst = os.path.join(conf[CONF_BUILDDIR], 'wingufile-%s' % conf[CONF_VERSION])
    ...
    tarball = os.path.join(conf[CONF_SRCDIR], 'wingufile-%s.tar.gz' % conf[CONF_WINGUFILE_VERSION])
    argv = [ 'tar', 'xf',
             tarball,
    ...
    if run_argv(argv) != 0:
        error('failed to uncompress wingufile')
...
def uncompress_ccnet():
    ...
    dst_dir = os.path.join(conf[CONF_BUILDDIR], 'wingufile-%s' % conf[CONF_VERSION], 'ccnet')
    must_mkdir(dst_dir)
    argv = [ 'tar', 'xf',
             tarball,
             '--strip-components=1',
...
def remove_debian_subdir():
    debian_subdir = os.path.join(conf[CONF_BUILDDIR], 'wingufile-%s' % conf[CONF_VERSION], 'debian')
...
def parse_args():
    ...
    parser.add_option(long_opt(CONF_VERSION),
                      dest=CONF_VERSION,
                      nargs=1,
                      help='the version of wingufile source. Must be digits delimited by dots, like 1.3.0')
...
def validate_args(usage, options):
    ...
    builddir = os.path.join(builddir, 'wingufile-deb-src')
