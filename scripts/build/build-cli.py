...
'''This scirpt builds the wingufile command line client (With no gui).
...
class Project(object):
    ...
    # Probject name, i.e. libseaprc/ccnet/wingufile/
    name = ''
    ...
    def __init__(self):
        ...
        self.prefix = os.path.join(conf[CONF_BUILDDIR], 'wingufile-cli')
...
def validate_args(usage, options):
    ...
    builddir = os.path.join(builddir, 'wingufile-cli-build')
...
def prepare_builddir(builddir):
    ...
    must_mkdir(os.path.join(builddir, 'wingufile-cli'))
...
def setup_build_env():
    ...
    prefix = os.path.join(conf[CONF_BUILDDIR], 'wingufile-cli')
...
def copy_scripts_and_libs():
    ...
    # copy the wrapper shell script for winguf-cli.py
    src = os.path.join(scripts_srcdir, 'winguf-cli-wrapper.sh')
    dst = os.path.join(cli_dir, 'winguf-cli')
    ...
    # rename winguf-cli to winguf-cli.py to avoid confusing users
    src = os.path.join(cli_dir, 'bin', 'winguf-cli')
    dst = os.path.join(cli_dir, 'bin', 'winguf-cli.py')
...
def get_dependent_libs(executable):
    syslibs = ['libwingurpc', 'libccnet', 'libwingufile', 'libpthread.so', 'libc.so', 'libm.so', 'librt.so', 'libdl.so', 'libselinux.so']
    def is_syslib(lib):
...
def copy_shared_libs():
    ...
    winguf_daemon_path = os.path.join(builddir,
                                    'wingufile-cli',
                                    'bin',
                                    'winguf-daemon')

    ccnet_daemon_libs = get_dependent_libs(ccnet_daemon_path)
    winguf_daemon_libs = get_dependent_libs(winguf_daemon_path)

    libs = ccnet_daemon_libs
    for lib in winguf_daemon_libs:
        if lib not in libs:
            libs.append(lib)
...
def strip_symbols():
    ...
    builddir = conf[CONF_BUILDDIR]
    topdir = os.path.join(builddir, 'wingufile-cli')
...
def create_tarball(tarball_name):
    ...
    cli_dir = 'wingufile-cli'
    versioned_cli_dir = 'wingufile-cli-' + version

    # move wingufile-cli to wingufile-cli-${version}
    try:
    ...
    ignored_patterns = [
        ...
        os.path.join(versioned_cli_dir, 'bin', 'wingurpc-codegen.py'),
        os.path.join(versioned_cli_dir, 'bin', 'wingufile-admin'),
        os.path.join(versioned_cli_dir, 'bin', 'wingufile'),
    ]
...
def gen_tarball():
    ...
    # 64-bit: wingufile-cli_1.2.2_x86-64.tar.gz
    # 32-bit: wingufile-cli_1.2.2_i386.tar.gz
    version = conf[CONF_VERSION]
    ...
    tarball_name = 'wingufile-cli_%(version)s_%(arch)s%(dbg)s.tar.gz' \
