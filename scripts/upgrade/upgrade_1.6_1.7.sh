...
echo
echo "------------------------------"
echo "Updating wingufile/winguhub database ..."
echo

winguhub_db=${TOPDIR}/winguhub.db
winguhub_sql=${UPGRADE_DIR}/sql/1.7.0/sqlite3/winguhub.sql
if ! sqlite3 "${winguhub_db}" < "${winguhub_sql}"; then
    echo "Failed to update winguhub database"
    exit 1
fi

wingufile_db=${wingufile_data_dir}/wingufile.db
wingufile_sql=${UPGRADE_DIR}/sql/1.7.0/sqlite3/wingufile.sql
if ! sqlite3 "${wingufile_db}" < "${wingufile_sql}"; then
    echo "Failed to update wingufile database"
    exit 1
fi
