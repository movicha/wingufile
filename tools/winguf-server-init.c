...
"  -d, --wingufile-dir      specify a diretory to put your wingufile server config and data\n" 
...
"       -r, --root-passwd      your mysql root passwd, needed to create wingufile server database\n"
...
int main (int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv, short_opts, long_opts, NULL)) != EOF) {
        ...
        case 'r':
            ...
            break;
        case 'd':
            config.wingufile_dir = strdup(optarg);
            break;
        case 'p':
    ...
    if (!config.wingufile_dir) {
        fprintf (stderr, "You must specify wingufile data dir\n");
        usage(1);
    }
    ...
    struct stat st;
    if (g_lstat (config.wingufile_dir, &st) < 0) {
        if (g_mkdir (config.wingufile_dir, 0777) < 0) {
            fprintf (stderr, "Directory %s cannot be created.\n", config.wingufile_dir);
            return 1;
        }
    }

    char *wingufile_conf = g_build_filename (config.wingufile_dir, "wingufile.conf", NULL);

    if (verbose)
        printf ("Generating config files:           %s\n", wingufile_conf);

    if (save_config_file (key_file, wingufile_conf) < 0)
        return 1;
