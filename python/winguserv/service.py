...
### Loading ccnet and wingufile configurations ###
'''ccnet'''
try:
...
'''wingufile'''
try:
    WINGUFILE_CONF_DIR = os.environ[ENVIRONMENT_VARIABLES[1]]
    if not WINGUFILE_CONF_DIR: # If it's set but is an empty string.
        raise KeyError
except KeyError:
    raise ImportError("Seaserv cannot be imported, because environment variable %s is undefined." % ENVIRONMENT_VARIABLES[1])
else:
    print "Loading wingufile config from " + WINGUFILE_CONF_DIR

WINGUFILE_CONF_DIR = os.path.normpath(os.path.expanduser(WINGUFILE_CONF_DIR))
config.read(os.path.join(WINGUFILE_CONF_DIR, 'wingufile.conf'))
