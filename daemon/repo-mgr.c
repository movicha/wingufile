...
#define IGNORE_FILE "wingufile-ignore.txt"

#ifdef HAVE_KEYSTORAGE_GK
#include "repokey/wingufile-gnome-keyring.h"
                                ...
                                char *root_id)
{
    ...
    if (passwd != NULL) {
        wingufile_generate_enc_key (passwd, strlen(passwd), 1, key, iv);
...
winguf_repo_index_commit (SeafRepo *repo, const char *desc, GError **error)
{
    ...
    if (read_index_from (&istate, index_path) < 0) {
        g_warning ("Failed to load index.\n");
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_INTERNAL, "Internal data structure error");
        return NULL;
    }
    ...
    if (index_add (repo, &istate, "") < 0) {
        g_set_error (error, WINGUFILE_DOMAIN, SEAF_ERR_GENERAL, "Failed to add");
        goto error;
    }
                           ...
                           char **error)
{
    ...
    if (repo->encrypted) {
        topts.crypt = wingufile_crypt_new (repo->enc_version, 
                                         repo->enc_key, 
                                         repo->enc_iv);
    }
...
recover_repo_enc_keys (SeafRepoManager *manager, SeafRepo *repo)
{
    ...
    wingufile_generate_enc_key (repo->passwd, strlen(repo->passwd), 
...
generate_repo_enc_key (SeafRepo *repo, const char *passwd)
{
     ...
     * We use large iteration times to defense against brute-force attack.
     */
    wingufile_generate_enc_key (passwd, strlen(passwd), repo->enc_version, key, iv);
