...
static GOptionEntry entries[] = 
{
    ...
    { .long_name            = "validate",
      ...
      .description          = "validate wingufile repos/branches/commits/blocks/indices",
      .arg_description      = NULL },
...
usage ()
{
    printf ("Usage:\n"
            ...
            "  -d, --wingufile-data-dir     set wingufile data directory\n"
            "  -w, --worktree-dir         set wingufile checkout files directory\n"
            "  -l, --list                 list all repos\n"
            "  --destroy repo-id          destory the repo with [repo_id]\n"
            "  --validate                 validate wingufile repos/branches/commits/blocks/indices\n"
...
get_winguf_data_dir (const char *dir)
{
    ...
    char *buf = g_build_filename(dir, WINGUFILE_INI, NULL);
    if (!g_file_test (buf, G_FILE_TEST_IS_REGULAR)) {
        fprintf (stdout, WINGUFILE_INI "not found in %s\n", dir);
        return NULL;
    }
...
int main(int argc, char *argv[])
{
    ...
    if (destroy_repo_id) {
        if (wingufile_is_running()) {
            fprintf (stderr, "wingufile is running, close it and try again.\n");
            return -1;
        }
