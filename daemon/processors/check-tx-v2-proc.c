...
{
    CCNET_PROCESSOR_CLASS (wingufile_check_tx_v2_proc_parent_class)->release_resource (processor);
}
...
encrypt_token (CcnetProcessor *processor, const char *token)
{
    ...
    if (!peer || !peer->session_key) {
        winguf_warning ("[check tx v2] peer or peer session key not exist\n");
        goto out;
    }

    wingufile_generate_enc_key (peer->session_key,
    ...
    if (wingufile_encrypt (&enc_out, &len, token, strlen(token) + 1, crypt) < 0) {
        winguf_warning ("[check tx v2] failed to encrypt token\n");
        goto out;
    }
...
start (CcnetProcessor *processor, int argc, char **argv)
{
    ...
    g_string_append_printf (buf,
              "remote %s wingufile-check-tx-slave-v2 %s %d %s %s %s",
