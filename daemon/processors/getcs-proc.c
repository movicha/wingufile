...
{
    CCNET_PROCESSOR_CLASS (wingufile_getcs_proc_parent_class)->release_resource (processor);
}
...
start (CcnetProcessor *processor, int argc, char **argv)
{
    ...
    snprintf (buf, 256, "remote %s wingufile-putcs", processor->peer_id);
...
add_chunk_server (CcnetProcessor *processor, TransferTask *task, char *cs_str)
{
    ...
    if (strcmp (cs_id, processor->peer_id) == 0) {
        CcnetPeer *peer = ccnet_get_peer (winguf->ccnetrpc_client,
                                          processor->peer_id);
        g_assert (peer);
        if (!peer->public_addr) {
