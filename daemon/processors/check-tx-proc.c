...
{
    CCNET_PROCESSOR_CLASS (wingufile_check_tx_proc_parent_class)->release_resource (processor);
}
...
start (CcnetProcessor *processor, int argc, char **argv)
{
    ...
    if (proc->type == CHECK_TX_TYPE_UPLOAD)
        snprintf (buf, sizeof(buf), 
                  "remote %s wingufile-check-tx-slave upload %d %s %s",
         ...
         */
        snprintf (buf, sizeof(buf), 
                  "remote %s wingufile-check-tx-slave download %d %s %s %s",
                  processor->peer_id, CURRENT_PROTO_VERSION, 
