...
/*
              wingufile-putcommit-v2 <HEAD> [END] (END is empty in clone)
  INIT      -------------------------->
                 OK
            <-------------------------
...
get_commit_start (CcnetProcessor *processor, int argc, char **argv)
{
    ...
    if (end_commit_id != NULL)
        g_string_printf (buf, "remote %s wingufile-putcommit-v2 %s %s %s",
                         processor->peer_id, 
                         task->head, end_commit_id, task->session_token);
    else
        g_string_printf (buf, "remote %s wingufile-putcommit-v2 %s %s",
                         processor->peer_id, 
