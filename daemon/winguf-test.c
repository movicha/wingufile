...
#define SEAF_DIR        CCNET_DIR "wingufile-data/"
...
static SeafRepo* create_repo (const char *repo_name)
{
    ...
    repo_id = wingufile_create_repo (repo_name, "test",
...
static const char* first_commit (SeafRepo *repo)
{
    ...
    if (wingufile_add (repo->id, "", &error) < 0) {
        fprintf (stderr, "Failed to add: %s.\n", error->message);
        return NULL;
    }

    commit_id = wingufile_commit (repo->id, "first commit", &error);
    if (commit_id == NULL) {
...
test_successful_merge ()
{
    ...
    /* create branch "test". */
    if (wingufile_branch_add (repo->id, "test", NULL, &error) < 0) {
    ...
    if (wingufile_checkout (repo->id, "test", &error) < 0) {
    ...
    if (wingufile_checkout (repo->id, "local", &error) < 0) {
    ...
    /* merge branch "test". */
    if (wingufile_merge (repo->id, "test", &error) < 0) {
    ...
    /* list commits */
    wingufile_get_commit_list (repo->id, 0, -1, NULL);
    wingufile_get_commit_list (repo->id, 0, -1, NULL);
