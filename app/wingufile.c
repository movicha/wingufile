...
{
    fputs (
"Usage: wingufile [--version ] [-c CONF_DIR] COMMAND [ARGS]\n"
"\n"
...
int main (int argc, char *argv[])
{
    ...
    context = g_option_context_new (NULL);
    g_option_context_add_main_entries (context, entries, "wingufile");
    ...
    g_log_set_default_handler (wingufile_log, NULL);
    ...
    rpc_client = ccnet_create_rpc_client (client, NULL, "wingufile-rpcserver");
...
static int create (int argc, char **argv)
{
    ...
    if (argc != 2 && argc != 3) {
        fprintf (stderr, "wingufile create [--encrypt] [--passwd=<passwd>] "
    ...
    char *maxsize_str = wingufile_get_config (rpc_client, key, &error);
    if (error) {
...
#define DEFAULT_MAX_REPO_BASE_SIZE 1024 /* in MB */
    ...
    guint repo_base_size = (guint)wingufile_calc_dir_size (rpc_client, base, &error);
    if (error) {
    ...
    repo_id = wingufile_create_repo (rpc_client, name, desc, base,
...
static int repo_remove (int argc, char **argv)
{
    ...
    if (argc != 1) {
        fprintf (stderr, "wingufile repo-rm <repo id>\n");
        return -1;
    }

    repo_id = argv[0];

    if (wingufile_destroy_repo (threaded_rpc_client, repo_id, &error) < 0) {
...
static int set_token (int argc, char **argv)
{
    ...
    if (argc < 2) {
        fprintf (stderr, "wingufile set-token <repo id> <token>\n");
        return -1;
    }
    ...
    if (wingufile_set_repo_token (rpc_client, repo_id, token, &error) < 0) {
...
static int get_token (int argc, char **argv)
{
    ...
    if (argc < 1) {
        fprintf (stderr, "wingufile get-token <repo id>\n");
        return -1;
    }

    repo_id = argv[0];

    token = wingufile_get_repo_token (rpc_client, repo_id, &error);
    if (!token) {
...
static int list_repos (int argc, char **argv)
{
    ...
    repos = wingufile_get_repo_list (rpc_client, -1, -1, &error);
...
static int set_auto (int argc, char **argv)
{
    ...
    if (argc != 1) {
        fprintf (stderr, "wingufile set-auto <repo id>\n");
        return -1;
    }
    repo_id = argv[0];

    int res = wingufile_set_repo_property (rpc_client, repo_id, "auto-sync", "true", &error);
    if (res != 0) {
...
static int set_manual (int argc, char **argv)
{
    ...
    if (argc != 1) {
        fprintf (stderr, "wingufile set-manual <repo id>\n");
        return -1;
    }
    repo_id = argv[0];

    int res = wingufile_set_repo_property (rpc_client, repo_id, "auto-sync", "false", &error);
    if (res != 0) {
...
static int list_worktrees (int argc, char **argv)
{
    ...
    if (argc != 0) {
        fprintf (stderr, "[usage] wingufile list-worktrees \n");
        return -1;
    }
