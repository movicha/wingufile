...
static int add_server (int argc, char **argv)
{
    ...
    if (wingufile_add_chunk_server (rpc_client, server_id, &error) < 0) {
...
static int del_server (int argc, char **argv)
{
    ...
    if (wingufile_del_chunk_server (rpc_client, server_id, &error) < 0) {
...
static int list_servers (int argc, char **argv)
{
    ...
    list = wingufile_list_chunk_servers (rpc_client, &error);
    if (!list) {
        fprintf (stderr, "%s\n", error->message);
        return -1;
    }
...
static int set_monitor (int argc, char **argv)
{
    ...
    if (wingufile_set_monitor (rpc_client, monitor_id, &error) < 0) {
...
static int get_monitor (int argc, char **argv)
{
    ...
    monitor_id = wingufile_get_monitor (rpc_client, &error);
    if (!monitor_id) {
...
static int put_file (int argc, char **argv)
{
    ...
    ret = wingufile_put_file(threaded_rpc_client, repo_id, file_path,
...
static int set_user_quota (int argc, char **argv)
{
    ...
    if (wingufile_set_user_quota (threaded_rpc_client, user, quota, &error) < 0) {
...
static int set_org_quota (int argc, char **argv)
{
    ...
    if (wingufile_set_org_quota (threaded_rpc_client, org_id, quota, &error) < 0) {
...
static int set_org_user_quota (int argc, char **argv)
{
    ...
    if (wingufile_set_org_user_quota (threaded_rpc_client, 
